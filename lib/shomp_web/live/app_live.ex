defmodule ShompWeb.AppLive do
  ...
  def handle_info({:purchase_completed, activity_data}, socket) do
    ...
    new_activities = [activity_data | socket.assigns.activities] |> Enum.take(3)

    # Schedule auto-removal after 8 seconds
    Process.send_after(self(), {:remove_activity, activity_data.id}, 8000)

    {:noreply, assign(socket, :activities, new_activities)}
  end

  @impl true
  def handle_info({:remove_activity, activity_id}, socket) do
    # Remove the activity from the list
    filtered_activities = Enum.reject(socket.assigns.activities, &(&1.id == activity_id))
